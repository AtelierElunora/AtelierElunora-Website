begin;
create table public.gallery_magnet_templates(
 event_id uuid primary key references public.gallery_events(id) on delete cascade,
 template jsonb not null check(jsonb_typeof(template)='object' and octet_length(template::text)<=12000),
 updated_at timestamptz not null default now()
);
alter table public.gallery_magnet_templates enable row level security;
revoke all on public.gallery_magnet_templates from public,anon,authenticated;
grant select,insert,update,delete on public.gallery_magnet_templates to service_role;
alter table public.gallery_print_jobs add column template jsonb check(template is null or (jsonb_typeof(template)='object' and octet_length(template::text)<=12000));
drop function public.gallery_print_update(text,uuid,integer,text,integer,numeric,numeric,numeric);
create function public.gallery_print_update(p_hash text,p_id uuid,p_version integer,p_action text,p_quantity integer default 1,p_x numeric default 50,p_y numeric default 50,p_zoom numeric default 1,p_template jsonb default null)
...
revoke all on function public.gallery_print_update(text,uuid,integer,text,integer,numeric,numeric,numeric,jsonb) from public,anon,authenticated;
grant execute on function public.gallery_print_update(text,uuid,integer,text,integer,numeric,numeric,numeric,jsonb) to service_role;
commit;
