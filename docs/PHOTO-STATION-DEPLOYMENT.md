...
## Magnet wrap templates — latest update

The owner subsequently confirmed PHOTO_STATION_ENABLED was saved and live capture → gallery → print desk plus capture-link revocation worked (user-reported acceptance). On September 20, added four-sided wrap templates. See MAGNET-WRAP-TEMPLATES.md for current deployment status and release instructions: database migration 20260920183216 applied; gallery-api v20 active; v1.35 theme staged from current v1.34; owner source ready for deployment/release. Prior pending rollout notes above are historical.
