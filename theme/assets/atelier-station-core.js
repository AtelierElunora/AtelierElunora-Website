...
export function normalizeTemplate(input={}){
 ...
 const t={enabled:input.enabled??false,photoCutInches:num('photoCutInches',2.5,2.5,3.75),cutInches:num('cutInches',3.25,3,3.75),fontSize:num('fontSize',7,5,12),edgeInset:num('edgeInset',0.12,0,0.625),background:color(input.background??'#EBE5D9'),color:color(input.color??'#252B1D'),company:text(input.company??'Atelier Elunora'),couple:text(input.couple??''),date:text(input.date??'',40),sides:{}};
 if(t.edgeInset>(t.cutInches-2.5)/2+0.000001)throw Error('Text distance from image edge must be zero or greater.');
 for(const side of templateSides){const row=input.sides?.[side]??{source:({top:'company',right:'date',bottom:'couple',left:'photo'})[side],text:'',offset:0,rotate:0};if(!['blank','custom','company','couple','date','event','photo'].includes(row.source))throw Error('Invalid text source.');const offset=row.offset??0,rotate=row.rotate??0,shift=row.shift??0;if(typeof offset!=='number'||!Number.isFinite(offset)||Math.abs(offset)>0.04||![0,180].includes(rotate))throw Error('Invalid side positioning.');if(typeof shift!=='number'||!Number.isFinite(shift)||Math.abs(shift)>0.5)throw Error('Along-edge text shift must be between −0.5 and 0.5 inches.');if(t.edgeInset+offset< -0.000001)throw Error('Side adjustment places the text center beyond the cut edge.');t.sides[side]={source:row.source,text:text(row.text??''),offset,rotate,shift};}
 return t;
}
...
export function drawMagnet(ctx,picture,crop,slot,template,context={},guides=false){
 ...
 templateSides.forEach((side,i)=>{const row=t.sides[side],text=resolveTemplateText(t,side,context);if(!text)return;ctx.save();ctx.translate(slot.size/2,slot.size/2);ctx.rotate(i*Math.PI/2);ctx.translate(row.shift*scale*(i<2?1:-1),-slot.size/2+(t.edgeInset+row.offset)*scale);ctx.rotate(row.rotate*Math.PI/180);ctx.fillText(text,0,0,face-0.3*scale);ctx.restore();});
