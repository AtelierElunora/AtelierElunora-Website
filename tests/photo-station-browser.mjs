...
p.document.getElementById('zoom').value=2;p.document.getElementById('zoom').dispatchEvent(new p.Event('input'));
assert.equal(p.document.getElementById('template-inset').value,'0.255');p.document.getElementById('template-inset').value='0';
...
p.document.getElementById('sheet-format').value='photo';p.document.getElementById('prepare').click();await settle();assert.equal(p.document.querySelectorAll('#sheets img').length,3);assert.deepEqual(updates,['claim']);assert.equal(claimZoom,2);assert.equal(claimTemplate.background,'#123456');assert.equal(claimTemplate.edgeInset,0.375);assert.equal(draws.at(-1)[3],450);
p.document.getElementById('print').click();assert.equal(prints,1);assert.deepEqual(updates,['claim']);p.document.getElementById('printed').click();await settle();assert.deepEqual(updates,['claim','printed']);assert.equal(p.document.querySelectorAll('#sheets img').length,0);state={...state,status:'pending',version:state.version+1,quantity:6};p.document.getElementById('refresh').click();await settle();p.document.querySelector('#jobs button').click();await settle();p.document.getElementById('sheet-format').value='letter';const beforeCopies=draws.length;p.document.getElementById('prepare').click();await settle();assert.equal(p.document.querySelectorAll('#sheets.letter-sheets img').length,1);assert.equal(draws.length-beforeCopies,6);assert.match(p.document.getElementById('notice').textContent,/1 Letter sheet/);p.happyDOM.abort();
