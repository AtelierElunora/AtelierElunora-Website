...
p.document.getElementById('prepare').click();await settle();assert.equal(p.document.querySelectorAll('#sheets img').length,3);assert.deepEqual(updates,['claim']);assert.equal(claimZoom,2);assert.equal(claimTemplate.background,'#123456');assert.equal(claimTemplate.edgeInset,0.375);assert.equal(draws.at(-1)[3],450);
