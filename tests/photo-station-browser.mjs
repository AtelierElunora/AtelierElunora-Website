...
function windowFor(purpose){const w=new Window({url:'https://www.atelierelunora.com/pages/photo-station#'+purpose+'='+'a'.repeat(64)});w.document.write(html);w.HTMLCanvasElement.prototype.getContext=()=>({drawImage(){},fillRect(){},fillText(){}});w.HTMLCanvasElement.prototype.toDataURL=()=> 'data:image/jpeg;base64,/9j/AAAA';return w;}
...
const p=windowFor('print');let state={id:'11111111-1111-4111-8111-111111111111',status:'pending',version:1,quantity:3,x:50,y:50,created_at:new Date().toISOString()},updates=[],prints=0,draws=[],claimZoom;
p.HTMLCanvasElement.prototype.getContext=()=>({drawImage(...args){draws.push(args);},fillRect(){},fillText(){}});
p.Image=class{naturalWidth=1200;naturalHeight=900;set src(v){queueMicrotask(()=>this.onload());}};p.print=()=>prints++;p.confirm=()=>true;
p.fetch=async(_,options)=>{const b=JSON.parse(options.body);let data;if(b.action==='info')data={name:'Print test'};if(b.action==='queue')data={jobs:state.status===b.status?[state]:[]};if(b.action==='image')data={url:'https://example.test/photo.jpg'};if(b.action==='update'){updates.push(b.operation);if(b.operation==='claim')claimZoom=b.zoom;state={...state,version:state.version+1,status:b.operation==='claim'?'printing':'printed'};data={job:state};}return {ok:true,json:async()=>data};};
...
p.document.getElementById('zoom').value=2;p.document.getElementById('zoom').dispatchEvent(new p.Event('input'));
p.document.getElementById('prepare').click();await settle();assert.equal(p.document.querySelectorAll('#sheets img').length,2);assert.deepEqual(updates,['claim']);assert.equal(claimZoom,2);assert.equal(draws.at(-1)[3],450);
...
console.log('PASS: simulated browser capture, approval, failed upload retry with same ID, reset/privacy, queue loading, 3-copy/2-sheet rendering, claim before print and explicit physical-print confirmation.');
